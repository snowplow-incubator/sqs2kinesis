...
object EventsStreamModule {
  ...
  def runStream(config: StreamConfig)(implicit system: ActorSystem) = {
    ...
    sqsSource
      .log("after sqsSource", x => s"incoming msg: $x")
      .asSourceWithContext(m => m)
      .log("after SourceWithContext", x => s"message md5: ${x.md5OfBody()}")
      .map(sqsMsg2kinesisMsg)
      .log("after sqsMsg2kinesisMsg", x => s"key: ${x._1}")
      .map(toPutRecordReqEntry)
      .log("after toPutRecordReqEntry", x => s"PutRecordReqEntry: $x")
      .via(kinesisFlow)
      .log("after kinesisFlow", x => s"PutRecordsResultEntry: $x")
      .asSource
      .map(_._2)
      .log("after asSource _._2", x => s"msg: $x")
      .alsoTo(confirmSqsSink)
      .log("after alsoTo", x => s"msg: $x")
